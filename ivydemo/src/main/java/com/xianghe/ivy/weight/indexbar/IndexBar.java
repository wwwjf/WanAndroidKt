package com.xianghe.ivy.weight.indexbar;
...
import com.xianghe.ivy.weight.indexbar.helper.IIndexBarDataHelper;
import com.xianghe.ivy.weight.indexbar.helper.IndexBarDataHelperImpl;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < mIndexDatas.size(); i++) {
            ...
            measureHeight = Math.max(indexBounds.height(), measureHeight) +
                    (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1,
                            getResources().getDisplayMetrics());//循环结束后，得到index的最大高度，然后*size
        }
        ...
        switch (hMode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                ...
                measureHeight = Math.max(measureHeight,hSize);
                break;
        }
