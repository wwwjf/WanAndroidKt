package com.xianghe.ivy.weight.indexbar.helper;
...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
    ...
    public IIndexBarDataHelper fillInexTag(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                //以下代码设置城市拼音首字母
                String baseIndexPinyin = indexPinyinBean.getBaseIndexPinyin();
                String tagString = "";
                if (!TextUtils.isEmpty(baseIndexPinyin)) {
                    tagString = baseIndexPinyin.substring(0, 1);
                }
...
//                    return lhs.getBaseIndexPinyin().compareTo(rhs.getBaseIndexPinyin());
                    if (lhs.getBaseIndexPinyin().compareTo(rhs.getBaseIndexPinyin()) > 0){
                        return 1;
