 ...
 * @Desc:
 */
class RespMapper : HttpRespMapper {
    override fun getInfoCode(resp: Any?): Int {
        return (resp as?BaseResponse<*>)?.infoCode ?: -1
    }

    override fun isSuccess(resp: Any?): Boolean {
        return resp != null &&
                resp is BaseResponse<*> &&
                (resp.status == RespCode.CODE_1)
//                && resp.data != null
    }

    override fun getErrorMessage(resp: Any?): String {
        return (resp as?BaseResponse<*>)?.info ?: "Http Request : UnknownError"
    }

    override fun getErrorCode(resp: Any?): Int {
        return (resp as?BaseResponse<*>)?.status ?: -1
    }
}