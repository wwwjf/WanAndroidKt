 ...
 * @Desc:
 */
abstract class BaseMVPCustomLoadingActivity<V : IBaseConctact.IBaseView, T : BasePresenter<V>> : BaseMVPActivity<V, T>() {
...
//    open fun displayHomeAsUpEnable(): Boolean = true
    ...
    open fun <E> onError(e: E?) {
        if (!this@BaseMVPCustomLoadingActivity.isFinishing) {
            if (e != null) {
                try {
                    parseError(e).let {
