...
void Synthesizer::ReceivePixel(uint8_t *rgba) {
    ...
    m_cur_v_frame = nullptr;
}

void Synthesizer::DecodeFinish(IDecoder *decoder) {
    // 编码结束，压入一帧空数据，通知编码器结束编码
    if (decoder == m_video_decoder) {
        m_v_encoder->PushFrame(new OneFrame(nullptr, 0, 0, AVRational{1, 25}, NULL));
    } else {
        m_a_encoder->PushFrame(new OneFrame(nullptr, 0, 0, AVRational{1, 25}, NULL));
    }
}
