...
void Mp4Muxer::Init(JNIEnv *env, jstring path) {
    const char *u_path = env->GetStringUTFChars(path, nullptr);
    ...
    //新建输出上下文
    avformat_alloc_output_context2(&m_fmt_ctx, nullptr, nullptr, m_path);
...
int Mp4Muxer::AddStream(AVCodecContext *ctx) {
    AVStream *video_stream = avformat_new_stream(m_fmt_ctx, nullptr);
