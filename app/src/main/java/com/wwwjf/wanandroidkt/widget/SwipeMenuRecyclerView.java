...
public class SwipeMenuRecyclerView extends RecyclerView {
    ...
    public static final String TAG = SwipeMenuRecyclerView.class.getSimpleName();
    ...
    public SwipeMenuRecyclerView(@NonNull @NotNull Context context) {
        super(context);
        init();
    }

    public SwipeMenuRecyclerView(@NonNull @NotNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SwipeMenuRecyclerView(@NonNull @NotNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mMoving){
                    ...
                    if (mMoveView.getScrollX() + dx >= 0 && mMoveView.getScrollX() + dx <= mMenuWidth) {
                        Log.e(TAG, "1-onTouchEvent: dx="+dx);
                    ...
                    if (isHorizontalMove) {
                        ...
                        //让itemView在规定区域随手指移动
                        if (mMoveView.getScrollX() + dx >= 0 && mMoveView.getScrollX() + dx <= mMenuWidth) {
                            Log.e(TAG, "2-onTouchEvent: dx="+dx);
                            mMoveView.scrollBy(dx, 0);
                        }
                        mLastX = x;
                        //设置正处于水平滑动状态
