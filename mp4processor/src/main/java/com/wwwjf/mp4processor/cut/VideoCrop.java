package com.wwwjf.mp4processor.cut;
...
import com.wwwjf.mp4processor.R;
import com.wwwjf.mp4processor.log.AvLog;
import com.wwwjf.mp4processor.utils.EGLUtils;
import com.wwwjf.mp4processor.utils.MediaCodecUtils;
...
public class VideoCrop {
    ...
    private boolean videoEnd = false; // 保证最后一帧执行完成之后再跳出去循环
    ...
    public void start(String inputVideoPath, String outputVideoPath, long startTime, long endTime, boolean swipeWH) {
        ...
        AvLog.d(TAG, "start inputVideoPath " + inputVideoPath + " \noutputVideoPath " + outputVideoPath
    ...
    private void startVideoCrop() {
        ...
        if (videoDecoder != null && videoEncode != null && mediaMuxer != null) {
            ...
            videoHandler.post(new Runnable() {
                @Override
                public void run() {
                    AvLog.d(TAG, "videoDecoder  start startTime " + startTime + " endTime " + endTime);
...
//                        AvLog.d(TAG, "run: videoDecoder  outIndex " + outIndex + " presentationTimeUs "
//                                + info.presentationTimeUs + " size " + info.size + " flag " + info.flags + " offset " + info.offset);
                            // 获取不到数据了就结束
                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                                AvLog.d(TAG, "run: videoDecoder  info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0");
                                videoEnd = true;
                            }
    ...
    //                                    AvLog.d(TAG, "run: videoDecoder  presentationTimeUsList.size： " + presentationTimeUsList.size());
                                    ...
                                    AvLog.i(TAG, "run: not draw");
                                    if (!outputStart) { // 不能output，暂时不处理
                                        presentationTimeUsList.add(presentationTimeUs);
                                    }
                                }
                                if (presentationTimeUs > endTime) {
                                    AvLog.d(TAG, "run: videoDecoder  > endTime  presentationTimeUs： " + presentationTimeUs);
...
//                            }
//                        }
//                        AvLog.d(TAG, "run: videoDecoder videoInputIndex " + videoInputIndex + " videoDrawIndex " + videoDrawIndex
//                                + " videoAllOutputIndex " + videoAllOutputIndex + " videoOutputIndex " + videoOutputIndex);
                        } catch (Exception e) {
                            e.printStackTrace();
                            if (encoderListener != null) {
                                encoderListener.onError(resources.getString(R.string.vc_exception_video_cut));
                    ...
                    AvLog.d(TAG, "run: videoDecoder release");
                    videoDecoder.stop();
...
//                        AvLog.d(TAG, "run: audioDecoder  outIndex " + outIndex + " presentationTimeUs "
//                                + info.presentationTimeUs + " size " + info.size + " flag " + info.flags + " offset " + info.offset);
                            ...
                            if (outIndex >= 0) {
                                ...
                                if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                                    info.size = 0;
    //                                AvLog.d(TAG, "run: audioDecoder size==0");
    ...
    //                                    AvLog.d(TAG, "run: audioDecoder presentationTimeUs "
    //                                            + info.presentationTimeUs + " size " + info.size + " flag " + info.flags + " offset " + info.offset);
    ...
    //                                    AvLog.d(TAG, "run: audioDecoder  > endTime");
                                        break;
                                    }
                                }
                            }
                            ...
                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
    //                            AvLog.d(TAG, "run: audioDecoder  info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0");
                                break;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                    ...
                    AvLog.d(TAG, "run: audioDecoder release");
                    audioDecoder.stop();
...
//                        AvLog.d(TAG, "run: audioEncode  outIndex " + outIndex + " presentationTimeUs "
//                                + info.presentationTimeUs + " size " + info.size + " flag " + info.flags + " offset " + info.offset);
                            ...
                            if (outIndex >= 0) {
                                ...
                                if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                                    info.size = 0;
    //                                AvLog.d(TAG, "run: audioEncode size==0");
    ...
    //                                AvLog.d(TAG, "run: audioEncode size!=0");
                                    ...
                                    if (presentationTimeUs >= startTime && presentationTimeUs <= endTime) {
                                        ...
                                        buffer.position(info.offset);
                                        buffer.limit(info.offset + info.size);
    //                                    AvLog.d(TAG, "run: audioEncode presentationTimeUs "
    //                                            + info.presentationTimeUs + " size " + info.size + " flag " + info.flags + " offset " + info.offset);
                                        // write
                                    ...
                                    audioEncode.releaseOutputBuffer(outIndex, false);
                                    if (presentationTimeUs > endTime) {
                                        AvLog.d(TAG, "run: audioEncode  > endTime  presentationTimeUs：" + presentationTimeUs);
                                        break;
                                    }
                                }
                                ...
                                if (mediaMuxer != null && audioTrackIndex == -1) {
                                    ...
                                    audioInit = true;
    //                                AvLog.d(TAG, "run: audioEncode   mediaMuxer.addTrack");
    //                                initMuxer();
                                }
                            }
                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                                AvLog.d(TAG, "run: audioEncode  info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0");
                                break;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                    ...
                    AvLog.d(TAG, "run: audioEncode release");
                    audioEncode.stop();
                    audioEncode.release();
                    audioEncode = null;
                    muxerRelease();
...
//        AvLog.d(TAG, "run: videoEncode  outIndex " + outIndex + " presentationTimeUs " + presentationTimeUs + " info.presentationTimeUs "
...
//                + info.flags + " offset " + info.offset);
        if (outIndex >= 0) {
            ...
            if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                info.size = 0;
//                                AvLog.d(TAG, "run: videoEncode size==0");
...
//                                AvLog.d(TAG, "run: videoEncode size!=0");
                if (presentationTimeUs >= startTime && presentationTimeUs <= endTime) {
                    ...
                    buffer.position(info.offset);
                    buffer.limit(info.offset + info.size);
//                                    AvLog.d(TAG, "run: videoEncode presentationTimeUs "
...
//                                            + " flag " + info.flags + " offset " + info.offset);
                ...
                videoEncode.releaseOutputBuffer(outIndex, false);
                                /*if (presentationTimeUs > endTime) {
//                                    AvLog.d(TAG, "run: videoEncode  > endTime");
                                    break;
                                }*/
            }
        } else if (outIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
            if (mediaMuxer != null && videoTrackIndex == -1) { // 保证执行一次
                ...
                videoInit = true;
//                                AvLog.d(TAG, "run: videoEncode   mediaMuxer.addTrack");
                initMuxer();
            }
        }
    }
        ...
        for (int i = 0; i < videoExtractor.getTrackCount(); i++) {
            ...
            if (mime.startsWith("video/")) {
               ...
               /* if (startTime != 0) {
                ...
                AvLog.d(TAG, "initVideoMediaCodec:   from format  videoTotalTime " + videoTotalTime);
                if (endTime == 0) {
                ...
                AvLog.d(TAG, "initVideoMediaCodec:  bitrate " + bit);
                ...
                try {
                    ...
                    AvLog.d(TAG, "initVideoMediaCodec:  rotation " + rotation + " width " + width + " height " + height);
                    ...
                    e.printStackTrace();
                    AvLog.e(TAG, "initVideoMediaCodec: Exception ");
                    swipeWH = sSwipeWH;
                }
                ...
                AvLog.d(TAG, "initVideoMediaCodec: swipeWH 是否置位 " + swipeWH + " inputVideoWidth " + inputVideoWidth + " inputVideoHeight  " + inputVideoHeight);
    ...
    private void initAudioMediaCodec() throws IOException {
        for (int i = 0; i < audioExtractor.getTrackCount(); i++) {
            ...
            if (mime.startsWith("audio/")) {
                ...
                AvLog.d(TAG, "initVideoMediaCodec:  audioTotalTime " + audioTotalTime);
                if (endTime == 0) {
